...
$EncryptedReceipt = "";
$DecryptedReceipt = $_POST['bank_response'];
	...
	if(trim($result_ca->card_number)==""){
		...
		if($digital_product == 1){
			...
			if($_SERVER["SERVER_NAME"]=="dev.shurjomukhi.com"){
				...
				$server_add = "http://localhost/shurjopay/";
			}
			else{
			...
			if($Transaction_amount > 500){
				$sql_query = mysqli_query($GLOBALS["___mysqli_sm"], "UPDATE sp_epay SET return_code='666', bank_status='MORE THAN 500' WHERE order_id='".$order_id."'");
				$_SESSION['order_details_response']['bankTxStatus'] = "FAIL";	
				$_SESSION['order_details_response']['txnAmount'] = $Transaction_amount;			
				$_SESSION['order_details_response']['spCode'] = "555";
				$_SESSION['order_details_response']['spCodeDes'] = "Amount is more than BDT 500";		
				return true;	
			}			
