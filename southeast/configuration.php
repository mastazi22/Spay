...
// Server name or IP address and port number of your proxy server
 //$configArray["proxyServer"] = "192.168.5.172:8080";
...
$configArray["merchantId"] = $_SESSION['ORDER_DETAILS']['eblID'];

// API username in the format below where Merchant ID is the same as above
$configArray["apiUsername"] = "merchant.".$_SESSION['ORDER_DETAILS']['eblID'];

// API password which can be configured in Merchant Administration
$configArray["password"] =$_SESSION['ORDER_DETAILS']['eblPassword'];// "ceba9d87930e1cee34f623a3e5ebc023";



		// Creating log
		$fp = fopen('log.txt', 'a');
		$response = time().'|'.$configArray["merchantId"].' '.$configArray["password"]."\n"; 
		fwrite($fp, $response);
		fclose($fp);
