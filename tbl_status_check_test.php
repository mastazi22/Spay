<?php
	...
	{	
		...
		{			
			...
			if (($CheckStatus->StatusText == 'REQ') && is_object($CheckStatus))
			{
	...
	*/
	...
	function GetOrderId($TxId) {
		//echo ("SELECT order_id FROM sp_epay WHERE txid='".$TxId."'");
	...
	*/
	function CheckStatus($OrderId,$MerchantId,$KeyCode) {
		...
		try {			
			...
			//return  json_decode(json_encode($ResponseData->TransactionInfo),TRUE); 
			//return  json_encode($ResponseData->TransactionInfo);
			//return $ResponseData->TransactionInfo;
