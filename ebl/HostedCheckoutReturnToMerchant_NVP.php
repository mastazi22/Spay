...
//$eblID    = $_SESSION['ORDER_DETAILS']['eblID'];
...
//if(isset($eblID) and $eblID !="") {
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $request_assoc_array = array("apiOperation" => "CREATE_CHECKOUT_SESSION",
	        ...
	        "interaction.cancelUrl" => "https://shurjopay.com/ebl/Cancelled.php?order=$order_id",
            "interaction.returnUrl" => "https://shurjopay.com/ebl/Receipt_NVP.php?order=$order_id"	
	    ...
	    $response = $parserObj->SendTransaction($merchantObj, $request, "POST");
        // Debuging
	...
	$sql_query = mysqli_query($GLOBALS["___mysqli_sm"],"UPDATE sp_epay SET bank_tx_id='" . $order_id . "' WHERE order_id='" . $sp_order_id . "'");
	   ...
	   $recieve_url = 'https://shurjopay.com/ebl/Receipt_NVP.php?resultIndicator='.$resultInd;
       header("Location: ".$recieve_url);
...
<head>
    ...
    <script src="https://easternbank.ap.gateway.mastercard.com/checkout/version/33/checkout.js"
            ...
            data-cancel="https://shurjopay.com/ebl/Cancelled.php"
            data-complete="https://shurjopay.com/ebl/Receipt_NVP.php"
    >
    </script>
        ...
        Checkout.configure({
            ...
            interaction: {
                merchant: {
                    name: 'shurjoPay',
                    address: {
                        line1: 'H# 320,R# 21, Mohakali(DOSH)',
                        line2: 'DHAKA 1206'
                    },
