...
#footer {
   ...
   <div id="main_container">
	   	...
	   	<form id="frm_submit" name="frm_submit" method="post" action="./payment_process_card_less.php">  
		  ...
		  <?
            ...
            foreach ($pe_methods as $method_key => $method_value) {
                if ($method_value['status'] == 'active' and $method_key!='master_card'):
                    ?>
                    <div class="form-radio-item" style="clear:left;padding-bottom: 30px;">
                        ...
                        <?php 
						if($method_key=="visa"){ ?>
			              <label for="input_5_<?php echo $method_key ?>" style="float: none; width: 50px; text-align: center;"><img src="img/<?php echo $method_value['img']; ?>" alt="<?php echo $method_value['text']; ?>" height="30"></label>&nbsp;<span style="color:#ff0000;font-weight:bold;padding-left: 7px;position: relative;top: -8px;font-size: 12px;">(Transaction limit: 2 per day and 4 per month)</span>
			             <? 
			             } 
			             else{
			             ?>
